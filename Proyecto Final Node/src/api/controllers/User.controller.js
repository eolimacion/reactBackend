...
const getFavTeams = async (req, res, next) => {
  try {
    const { id } = req.user;
    const userById = await User.findById(id).populate('favTeams'); // Populate favAlbums
    if (userById) {
      return res.status(200).json(userById.favTeams);
    } else {
...
const getFavLifters = async (req, res) => {
  try {
    const { id } = req.user;
    const userById = await User.findById(id).populate('favLifters'); // Populate favAlbums
    if (userById) {
      return res.status(200).json(userById.favLifters);
    } else {
      return res.status(404).json("That user doesn't exist.");
    }
  } catch (error) {
